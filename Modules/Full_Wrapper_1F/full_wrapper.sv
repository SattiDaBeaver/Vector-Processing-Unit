module top_wrapper #(
    ...
    // Mode select: 0 = instruction mem, 1 = internal mem
    input  logic mem_internal_mode,

    // FSM external control
    input  logic step,  
    ...
    //======================================//
    logic [INSTR_WIDTH-1:0]         instr_rd_data;
    logic [$clog2(INSTR_DEPTH)-1:0] instr_rd_addr;

    logic instr_mem_rst;
    logic instr_uart_rx;

    assign instr_mem_rst = rst | rst_mem_ptr;
    assign instr_uart_rx = (mem_internal_mode) ? 1'b1 : uart_rx;
    ...
    uart_instr_mem_loader #(
        ...
        .clk(clk),
        .rst(instr_mem_rst),
        .uart_rx(instr_uart_rx),
        .uart_tx(),  // optional echo
        .rd_addr(instr_rd_addr),
        .rd_data(instr_rd_data)
    );

    //======================================//
    // Internal Memory via UART
    //======================================//
    logic [DP_ADDR_WIDTH-1:0] mem_uart_addr;
    logic [DATA_WIDTH-1:0]    mem_uart_din;
    logic                     mem_uart_wr_en;
    logic [DATA_WIDTH-1:0]    mem_int [0:(1<<DP_ADDR_WIDTH)-1];
    
    // UART RX -> internal memory (8-bit)
    always_ff @(posedge clk) begin
        if (rst | rst_mem_ptr) begin
            mem_uart_addr <= '0;
        end else if (mem_uart_wr_en) begin
            mem_int[mem_uart_addr] <= mem_uart_din;
            mem_uart_addr <= mem_uart_addr + 1;
        end
    end

    //======================================//
    // UART Wrapper for Internal Memory RX/TX
    //======================================//
    logic uart_tx_busy;
    ...
    UART_wrapper #(
        ...
        .RX_dataIn(uart_rx),
        .TX_out(uart_tx),
        .TX_done(),      // can tie to flags if needed
        .TX_busy(uart_tx_busy),

        .RX_dataOut(mem_uart_din),
        .RX_done(mem_uart_wr_en),
        .RX_parityError() // ignored
    );

    //======================================//
    // Mux internal vs external FSM memory
    //======================================//
    logic                        mem_ext_en_mux;
    logic                        mem_wr_en_mux;
    logic [DP_ADDR_WIDTH-1:0]    mem_addr_mux;
    logic [DATA_WIDTH-1:0]       mem_din_mux;

    always_comb begin
        if (mem_internal_mode) begin
            mem_ext_en_mux = 1'b1;
            mem_wr_en_mux  = mem_uart_wr_en;
            mem_addr_mux   = mem_uart_addr;
            mem_din_mux    = mem_uart_din;
        end else begin
            mem_ext_en_mux = 1'b0;
            mem_wr_en_mux  = 1'b0;
            mem_addr_mux   = '0;
    ...
    tiny_fsm_control #(
        ...
        .mem_ext_en(mem_ext_en_mux),
        .mem_wr_en(mem_wr_en_mux),
        .mem_ext_addr(mem_addr_mux),
        .mem_ext_din(mem_din_mux),
        .mem_ext_dout(uart_tx_data), // optional read-back
